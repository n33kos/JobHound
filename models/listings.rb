...
def get_new_listings database_file, orderby, direction
	if File.file?(database_file)
		query = "select * from listings where (dismissed_bit = 0 or dismissed_bit = null or dismissed_bit is null) and (status is null or status = \"\")"
		if !orderby.nil?
			query += " order by "+orderby
		end
		if !direction.nil?
